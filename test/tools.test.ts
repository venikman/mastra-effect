...
describe("tools", () => {
  it("denies reading .env", async () => {
    ...
    await expect(tools.readFile.execute!({ path: ".env" } as any, undefined as any)).rejects.toMatchObject({
  ...
  it("truncates readFile with maxBytes", async () => {
    ...
    const out = await tools.readFile.execute!({ path: "big.txt", maxBytes: 10 } as any, undefined as any);

    expect("truncated" in out).toBe(true);
    if (!("truncated" in out)) throw new Error("Unexpected validation error");
